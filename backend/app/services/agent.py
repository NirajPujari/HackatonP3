...
def score_llm_match(input_word: str, translated_word: str, db_record_name: str, came_from_cache: bool, llm_output_raw: str) -> dict:
    ...
    if dist <= 2:
        return {"confidence": 0.75 if dist <= 1 else 0.60, "match_type": "transliteration"}
        
    hedges = ["unavailable", "approximately", "similar to", "not available", "no translation", "unknown", "n/a"]
    lower_llm_output = llm_output_raw.lower()
    if not any(h in lower_llm_output for h in hedges):
        return {"confidence": 0.95 if came_from_cache else 0.85, "match_type": "semantic"}
...
def deduplication_agent(input_word: str):
    ...
    if l1_results:
        return {
            ...
            "duplicate_languages": list(set(d["lang"] for d in l1_results if d.get("lang"))),
    ...
    if l2_results:
        return {
            ...
            "duplicate_languages": list(set(d["lang"] for d in l2_results if d.get("lang"))),
    ...
    for iteration in range(5):
        ...
        unique_results = [v["translation"] for k, v in current_translations.items() if "unavailable" not in v["translation"].lower()]
        failed_langs = [k for k, v in current_translations.items() if "unavailable" in v["translation"].lower()]
    ...
    if duplicates:
        ...
        if normalized_input in duplicate_names:
            ...
            score = score_llm_match(input_word, normalized_input, dup_doc["name"], False, normalized_input)
        ...
        for lang, trans_info in current_translations.items():
            ...
            if trans_word.lower() in duplicate_names:
                ...
                if not any(fd["name"] == dup_doc["name"] and fd["lang"] == dup_doc.get("lang", "") for fd in formatted_duplicates):
                    formatted_duplicates.append({
                        "name": dup_doc["name"],
                        "lang": dup_doc.get("lang", ""),
                        "confidence": score["confidence"],
